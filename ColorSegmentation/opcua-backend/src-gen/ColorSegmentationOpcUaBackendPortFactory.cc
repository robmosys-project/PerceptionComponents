...
#include "ColorSegmentationOpcUaBackendPortFactory.hh"
...
static ColorSegmentationOpcUaBackendPortFactory OpcUaBackendPortFactory;

ColorSegmentationOpcUaBackendPortFactory::ColorSegmentationOpcUaBackendPortFactory()
{  
	componentImpl = 0;
	ColorSegmentation::instance()->addPortFactory("OpcUa_SeRoNet", this);
}

ColorSegmentationOpcUaBackendPortFactory::~ColorSegmentationOpcUaBackendPortFactory()
{  }

void ColorSegmentationOpcUaBackendPortFactory::initialize(ColorSegmentation *component, int argc, char* argv[])
{
	componentImpl = new SeRoNet::Utils::Component(component->connections.component.name);
}

int ColorSegmentationOpcUaBackendPortFactory::onStartup()
{
	if (!component_thread.joinable()) {
    	component_thread = std::thread(&ColorSegmentationOpcUaBackendPortFactory::task_execution, this);
    	return 0;
    }
	return -1;
}

Smart::IPushClientPattern<DomainVision::CommVideoImage> * ColorSegmentationOpcUaBackendPortFactory::createRGBImagePushServiceIn()
{
	return new SeRoNet::OPCUA::Client::PushClient<DomainVision::CommVideoImage>(componentImpl);
}


Smart::IQueryServerPattern<CommPerception::CommInfDetection, CommPerception::CommObjectProperties> * ColorSegmentationOpcUaBackendPortFactory::createObjectQueryServiceAnsw(const std::string &serviceName)
{
...
int ColorSegmentationOpcUaBackendPortFactory::task_execution()
{
	componentImpl->run();
	return 0;
}

int ColorSegmentationOpcUaBackendPortFactory::onShutdown(const std::chrono::steady_clock::duration &timeoutTime)
{
...
void ColorSegmentationOpcUaBackendPortFactory::destroy()
{
