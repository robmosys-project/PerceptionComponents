...
SpinCore::SpinCore(Smart::IComponent *comp, const bool &useDefaultState) 
...
void SpinCore::notify_all_interaction_observers() {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	// try dynamically down-casting this class to the derived class 
	// (we can do it safely here as we exactly know the derived class)
	if(const Spin* spin = dynamic_cast<const Spin*>(this)) {
		for(auto it=interaction_observers.begin(); it!=interaction_observers.end(); it++) {
			(*it)->on_update_from(spin);
		}
	}
}

void SpinCore::attach_interaction_observer(SpinObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.push_back(observer);
}

void SpinCore::detach_interaction_observer(SpinObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.remove(observer);
}

int SpinCore::execute_protected_region()
{
...
void SpinCore::updateAllCommObjects()
{
...
void SpinCore::triggerLogEntry(const int& idOffset)
{
	if(useLogging == true) {
...
int SpinCore::getPreviousCommObjId()
{
