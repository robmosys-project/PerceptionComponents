...
#include "ROSBridgeOpcUaBackendPortFactory.hh"
...
ROSBridgeOpcUaBackendPortFactory::ROSBridgeOpcUaBackendPortFactory()
{  
	componentImpl = 0;
	ROSBridge::instance()->addPortFactory("OpcUa_SeRoNet", this);
}

ROSBridgeOpcUaBackendPortFactory::~ROSBridgeOpcUaBackendPortFactory()
{  }

void ROSBridgeOpcUaBackendPortFactory::initialize(ROSBridge *component, int argc, char* argv[])
{
	componentImpl = new SeRoNet::Utils::Component(component->connections.component.name);
}

int ROSBridgeOpcUaBackendPortFactory::onStartup()
{
	if (!component_thread.joinable()) {
    	component_thread = std::thread(&ROSBridgeOpcUaBackendPortFactory::task_execution, this);
    	return 0;
    }
	return -1;
}
...
int ROSBridgeOpcUaBackendPortFactory::task_execution()
{
	componentImpl->run();
	return 0;
}

int ROSBridgeOpcUaBackendPortFactory::onShutdown(const std::chrono::steady_clock::duration &timeoutTime)
{
...
void ROSBridgeOpcUaBackendPortFactory::destroy()
{
