...
CaptureSensorCore::CaptureSensorCore(Smart::IComponent *comp, const bool &useDefaultState) 
...
CaptureSensorCore::~CaptureSensorCore()
{
}


void CaptureSensorCore::notify_all_interaction_observers() {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	// try dynamically down-casting this class to the derived class 
	// (we can do it safely here as we exactly know the derived class)
	if(const CaptureSensor* captureSensor = dynamic_cast<const CaptureSensor*>(this)) {
		for(auto it=interaction_observers.begin(); it!=interaction_observers.end(); it++) {
			(*it)->on_update_from(captureSensor);
		}
	}
}

void CaptureSensorCore::attach_interaction_observer(CaptureSensorObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.push_back(observer);
}

void CaptureSensorCore::detach_interaction_observer(CaptureSensorObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.remove(observer);
}

int CaptureSensorCore::execute_protected_region()
{
...
Smart::StatusCode CaptureSensorCore::rGBImagePushServiceOutPut(DomainVision::CommVideoImage &rGBImagePushServiceOutDataObject)
{
...
void CaptureSensorCore::triggerLogEntry(const int& idOffset)
{
	if(useLogging == true) {
...
int CaptureSensorCore::getPreviousCommObjId()
{
