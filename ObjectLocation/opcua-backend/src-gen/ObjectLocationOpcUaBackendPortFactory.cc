...
#include "ObjectLocationOpcUaBackendPortFactory.hh"
...
static ObjectLocationOpcUaBackendPortFactory OpcUaBackendPortFactory;

ObjectLocationOpcUaBackendPortFactory::ObjectLocationOpcUaBackendPortFactory()
{  
	componentImpl = 0;
	ObjectLocation::instance()->addPortFactory("OpcUa_SeRoNet", this);
}

ObjectLocationOpcUaBackendPortFactory::~ObjectLocationOpcUaBackendPortFactory()
{  }

void ObjectLocationOpcUaBackendPortFactory::initialize(ObjectLocation *component, int argc, char* argv[])
{
	componentImpl = new SeRoNet::Utils::Component(component->connections.component.name);
}

int ObjectLocationOpcUaBackendPortFactory::onStartup()
{
	if (!component_thread.joinable()) {
    	component_thread = std::thread(&ObjectLocationOpcUaBackendPortFactory::task_execution, this);
    	return 0;
    }
	return -1;
}

Smart::IPushClientPattern<DomainVision::CommRGBDImage> * ObjectLocationOpcUaBackendPortFactory::createRGBDImagePushServiceIn()
{
	return new SeRoNet::OPCUA::Client::PushClient<DomainVision::CommRGBDImage>(componentImpl);
}


Smart::IQueryServerPattern<CommPerception::CommInfDetection, CommPerception::CommObjectProperties> * ObjectLocationOpcUaBackendPortFactory::createObjectQueryServiceAnsw(const std::string &serviceName)
{
...
int ObjectLocationOpcUaBackendPortFactory::task_execution()
{
	componentImpl->run();
	return 0;
}

int ObjectLocationOpcUaBackendPortFactory::onShutdown(const std::chrono::steady_clock::duration &timeoutTime)
{
...
void ObjectLocationOpcUaBackendPortFactory::destroy()
{
